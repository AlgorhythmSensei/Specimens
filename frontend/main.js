...
let selectedId = null;
let focusedId = null;
const colors = { cafe: '#f2b56b', bar: '#d79b88', church: '#b8a8cf', forest: '#9bbd91', homes: '#c7b9a2', pop_up: '#e8d85d' };
const venueDescriptions = {
  ...
  pop_up: ['A travelling social event has appeared here.', 'Tonight: a lantern circle and stories traded for hot soup.', 'Tonight: a rooftop vinyl exchange with improvised dancing.', 'Tonight: a midnight astronomy club looking for strange stars.'],
};

function zoneTitle(name) { return name === 'pop_up' ? 'SOCIAL EVENT' : name.replace('_', ' ').toUpperCase(); }
function zoneDescription(zone, packet) { const descriptions = venueDescriptions[zone.name] || ['A place of uncertain significance.']; return descriptions[Math.floor(packet.tick / 40) % descriptions.length]; }
...
function render(packet) {
  ...
  packet.animals.forEach(animal => { const trail = animalTrails.get(animal.id) || []; const isBear = animal.species === 'bear'; if (trail.length > 1) { ctx.beginPath(); trail.forEach((point, index) => index ? ctx.lineTo(point.x, point.y) : ctx.moveTo(point.x, point.y)); ctx.strokeStyle = isBear ? 'rgba(63,40,28,.45)' : 'rgba(139,84,47,.35)'; ctx.lineWidth = isBear ? 3 : 2; ctx.stroke(); } ctx.fillStyle = isBear ? '#4a3024' : '#8b542f'; ctx.beginPath(); ctx.ellipse(animal.x, animal.y, isBear ? 10 : 7, isBear ? 7 : 4, 0, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(animal.x + (isBear ? 9 : 7), animal.y - 2, isBear ? 4 : 3, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = isBear ? '#4a3024' : '#8b542f'; ctx.lineWidth = 1.5; [-4, 1].forEach(offset => { ctx.beginPath(); ctx.moveTo(animal.x + offset, animal.y + 2); ctx.lineTo(animal.x + offset - 1, animal.y + 6); ctx.stroke(); }); if (animal.sleeping) { ctx.font = '500 12px DM Mono'; ctx.fillStyle = '#536057'; ctx.fillText('Zzz', animal.x + 10, animal.y - 8); } if (isBear && animal.mad) { ctx.font = '500 11px DM Mono'; ctx.fillStyle = '#9c3f54'; ctx.fillText('MAD', animal.x + 13, animal.y + 13); } });
  ctx.beginPath(); ctx.arc(packet.teleporter.x, packet.teleporter.y, 13 + Math.sin(animationFrame / 8) * 3, 0, Math.PI * 2); ctx.fillStyle = '#d9f264'; ctx.shadowColor = '#d9f264'; ctx.shadowBlur = 25; ctx.fill(); ctx.shadowBlur = 0;
  packet.specimens.forEach(specimen => {
    const trail = trails.get(specimen.id) || [];
    if (trail.length > 1) { ctx.beginPath(); trail.forEach((point, index) => index ? ctx.lineTo(point.x, point.y) : ctx.moveTo(point.x, point.y)); ctx.strokeStyle = specimen.gender === 'man' ? 'rgba(232,91,57,.3)' : 'rgba(53,111,120,.3)'; ctx.lineWidth = 2; ctx.stroke(); }
    ...
    if (specimen.new_arrival) { ctx.beginPath(); ctx.arc(specimen.x, specimen.y, radius + 5 + Math.sin(animationFrame / 4) * 2, 0, Math.PI * 2); ctx.strokeStyle = '#f06b3f'; ctx.lineWidth = 2; ctx.stroke(); ctx.font = '500 11px DM Mono'; ctx.fillStyle = '#f06b3f'; ctx.fillText('NEW', specimen.x + radius + 9, specimen.y + radius + 13); }
  ...
  document.querySelector('#simulation-label').textContent = `TEST SIMULATION #${packet.simulation_number}`;
  const events = document.querySelector('#events'); const notable = packet.specimens.filter(s => ['teleported', 'conflict', 'reproduce'].includes(s.action)).slice(0, 4); events.innerHTML = notable.length ? notable.map(s => `<div class="event"><b>#${s.id}</b> ${s.action}</div>`).join('') : '<div class="event">Population moving through the field.</div>';
...
function updateAgentCards(packet) {
  ...
  if (resource) zoneCard.innerHTML = resource.kind === 'animal' ? `<b>${resource.species.toUpperCase()} #${resource.id}</b><span>${resource.species === 'bear' ? (resource.mad ? 'MAD BEAR: it has eaten poison and will attack nearby deer, plants, and specimens for two simulated hours.' : 'A bear predator. It eats deer, plants, and, when enraged, nearby specimens.') : 'A deer herbivore. It roams the forest and feeds on plants.'}</span>` : `<b>PLANT #${resource.id}</b><span>${resource.poisonous ? 'POISONOUS PLANT: it can kill an agent that eats it.' : 'A safe plant. It stays rooted, regrows naturally, and can feed any agent.'}</span>`;
...
canvas.addEventListener('pointermove', event => {
  ...
  canvas.style.cursor = hit || resourceHit || teleporterHit ? 'pointer' : 'default';
  const hoverCard = document.querySelector('#hover-card');
  const zoneCard = document.querySelector('#zone-card');
  ...
  zoneCard.dataset.resourceId = resourceHit && !hit && !teleporterHit ? String(resourceHit.resource.id) : '';
  zoneCard.dataset.teleporter = String(teleporterHit && !hit && !resourceHit);
  if ((zone || teleporterHit || resourceHit) && !hit) { zoneCard.style.left = `${event.offsetX + 16}px`; zoneCard.style.top = `${event.offsetY + 16}px`; }
  updateAgentCards(previousPacket);
});
canvas.addEventListener('pointerleave', () => { hoveredId = null; document.querySelector('#hover-card').setAttribute('aria-hidden', 'true'); document.querySelector('#zone-card').setAttribute('aria-hidden', 'true'); canvas.style.cursor = 'default'; });
...
document.querySelector('#pause').onclick = event => { window.simSocket?.send('toggle'); state.running = !state.running; event.currentTarget.textContent = state.running ? 'Pause simulation' : 'Resume simulation'; };
